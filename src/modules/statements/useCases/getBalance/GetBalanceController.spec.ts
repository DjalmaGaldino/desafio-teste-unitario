...
describe("Get Balance Controller", () => {
  ...
  it("should be able to get a balance", async () => {
    ...
    const statement2 = await request(app).post("/api/v1/statements/withdraw").send({
        amount: 50,
        description: "Withdraw test",
      }).set({ Authorization: `Bearer ${token}` });

    const response = await request(app).get("/api/v1/statements/balance").send().set({ Authorization: `Bearer ${token}` });
